"""Repository analysis tools for code review."""
...
class RepositoryAnalyzer:
    """Repository analyzer for deep code understanding."""
    ...
    async def analyze_repository(
        self, repository: str, head_sha: str, base_sha: str, max_files: int = 100
    ) -> dict:
        ...
        try:
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                file_contents = await self._get_relevant_file_contents(
                    repo_path, changed_files, project_structure, max_files
                )
                ...
                return {
                    ...
                    "analysis_summary": {
                        ...
                        "project_type": project_structure.get(
                            "project_type", "unknown"
                        ),
                        "main_language": project_structure.get(
    ...
    async def _clone_repository(self, repository: str, repo_path: Path, sha: str):
        ...
        try:
            ...
            try:
                ...
                self.logger.warning(
                    "Could not checkout specific SHA, trying to fetch more",
                    error=str(e),
                )
    ...
    async def _analyze_project_structure(self, repo_path: Path) -> dict:
        ...
        try:
            ...
            structure["directory_structure"] = await self._scan_directory_structure(
                repo_path
            )
    ...
    async def _scan_directory_structure(
        self, repo_path: Path, max_depth: int = 3
    ) -> dict:
        ...
        for file_path in repo_path.rglob("*"):
            if file_path.is_file():
                ...
                if file_path.suffix.lower() in {".md", ".rst", ".txt"} and any(
                    doc_keyword in file_path.name.lower()
    ...
    async def _get_relevant_file_contents(
        ...
        for entry_point in entry_points:
            ...
            if entry_path.exists() and str(entry_point) not in [
                f[0] for f in files_to_analyze
            ]:
        ...
        for file_path, file_type, priority in priority_files:
            async with self.throttler:
                try:
                    full_path = repo_path / file_path
                    if (
                        full_path.exists() and full_path.stat().st_size < 500_000
                    ):  # 500KB limit per file
                    ...
                    self.logger.warning(
                        "Failed to read file", file=file_path, error=str(e)
                    )
    ...
    def _should_analyze_file(self, file_path: Path) -> bool:
        ...
        if file_path.suffix.lower() not in self.code_extensions:
            ...
            if (
                file_path.suffix == ""
                and file_path.name.lower() not in self.config_files
            ):
                return False
