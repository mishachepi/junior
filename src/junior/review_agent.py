...
from .models import ReviewComment, ReviewCategory, Severity
...
class LogicalReviewAgent:
        ...
        try:
            ...
            for finding_data in findings_list:
                try:
                    ...
                    self.logger.warning("Failed to parse finding", 
                                      finding=finding_data, 
                                      error=str(finding_error))
                    
        except Exception as e:
            self.logger.error("Failed to parse AI response", category=category, error=str(e))
        
        return findings
    
    async def _analyze_project_logic(self, state: LogicalReviewState) -> LogicalReviewState:
        """Analyze overall project logic and architecture."""
        self.logger.info("Analyzing project logic", pr=state.pr_number)
        
        prompt = ChatPromptTemplate.from_messages([
            SystemMessage(content="""You are a senior software architect reviewing code changes.
            ...
            HumanMessage(content=f"""Project: {state.repository}
            PR: #{state.pr_number}
        ...
        try:
            ...
            self.logger.info("Logic analysis completed", 
    ...
    async def _check_logical_security(self, state: LogicalReviewState) -> LogicalReviewState:
        """Check for logical security vulnerabilities."""
        self.logger.info("Checking logical security", pr=state.pr_number)
        
        prompt = ChatPromptTemplate.from_messages([
            SystemMessage(content="""You are a security expert focusing on logical vulnerabilities.
            ...
            HumanMessage(content=f"""Analyze this diff for logical security vulnerabilities:
    ...
    async def _find_critical_bugs(self, state: LogicalReviewState) -> LogicalReviewState:
        """Find critical bugs and potential zero-days."""
        self.logger.info("Finding critical bugs", pr=state.pr_number)
        
        prompt = ChatPromptTemplate.from_messages([
            SystemMessage(content="""You are a bug hunter specializing in critical vulnerabilities.
            Search for:
            ...
            HumanMessage(content=f"""Hunt for critical bugs in this code change:
        ...
        try:
            ...
            self.logger.info("Critical bug analysis completed", 
                           findings_count=len(findings),
                           pr=state.pr_number)
    ...
    async def _review_naming_conventions(self, state: LogicalReviewState) -> LogicalReviewState:
        """Review naming conventions that impact readability."""
        self.logger.info("Reviewing naming conventions", pr=state.pr_number)
        
        prompt = ChatPromptTemplate.from_messages([
            SystemMessage(content="""Review naming conventions for readability and maintainability.
            ...
            HumanMessage(content=f"""Review naming in this code change:
        ...
        try:
            ...
            self.logger.info("Naming review completed", 
                           findings_count=len(findings),
                           pr=state.pr_number)
    ...
    async def _check_code_optimization(self, state: LogicalReviewState) -> LogicalReviewState:
        """Check for code optimization opportunities."""
        self.logger.info("Checking optimization opportunities", pr=state.pr_number)
        
        prompt = ChatPromptTemplate.from_messages([
            SystemMessage(content="""Analyze code for significant optimization opportunities:
            ...
            HumanMessage(content=f"""Analyze optimization opportunities in:
    ...
    async def _verify_design_principles(self, state: LogicalReviewState) -> LogicalReviewState:
        """Verify adherence to DRY and KISS principles."""
        self.logger.info("Verifying design principles", pr=state.pr_number)
        
        prompt = ChatPromptTemplate.from_messages([
            SystemMessage(content="""Evaluate adherence to core design principles:
            ...
            HumanMessage(content=f"""Evaluate design principle adherence in:
        ...
        try:
            ...
            self.logger.info("Design principles analysis completed", 
                           findings_count=len(findings),
                           pr=state.pr_number)
    ...
    async def _generate_review_summary(self, state: LogicalReviewState) -> LogicalReviewState:
        """Generate comprehensive review summary."""
        self.logger.info("Generating review summary", pr=state.pr_number)
        ...
        prompt = ChatPromptTemplate.from_messages([
            SystemMessage(content="""Generate a comprehensive code review summary.
            ...
            HumanMessage(content=f"""Create review summary for PR #{state.pr_number}:
        ...
        try:
            ...
            for finding in state.findings:
                ...
                comment = ReviewComment(
                    category=category_map.get(finding.category.lower(), ReviewCategory.LOGIC),
    ...
    async def review_pull_request(
        ...
        self.logger.info("Starting logical review", 
        ...
        try:
            ...
            return {
                ...
                "comments": [comment.dict() for comment in getattr(final_state, 'review_comments', [])],
