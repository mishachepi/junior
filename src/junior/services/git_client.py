...
class GitClient:
            ...
            if file_path.is_file():
                files.append(
                    FileChange(
                        ...
                        if self._should_read_content(untracked)
                        else None,
                    )
                )
    ...
    def _diff_item_to_file_change(self, diff_item) -> FileChange | None:
    ...
    def get_commit_info(self, commit_sha: str | None = None) -> dict:
        """Get information about a specific commit."""
        commit = (
            self.repo.head.commit
            if commit_sha is None
            else self.repo.commit(commit_sha)
        )
